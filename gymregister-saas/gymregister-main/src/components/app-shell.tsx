...
import { Dumbbell, LayoutDashboard, Users, UserPlus, LogOut, Menu, X } from "lucide-react";
...
const nav = [
  ...
  { to: "/members", label: "Socios", icon: Users },
  { to: "/members/new", label: "Registrar", icon: UserPlus },
] as const;
...
export function AppShell({ children }: { children: ReactNode }) {
  ...
  const gymName = profile.gyms?.name;
      ...
      <aside className="hidden lg:flex w-64 shrink-0 flex-col border-r border-sidebar-border bg-sidebar">
        <div className="h-16 flex items-center gap-2 px-6 border-b border-sidebar-border">
          ...
          <div className="min-w-0">
            ...
            {gymName && (
              <span className="text-xs text-muted-foreground block truncate">{gymName}</span>
            )}
          </div>
        </div>
